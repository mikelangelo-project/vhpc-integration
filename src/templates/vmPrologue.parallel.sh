...
| cut -d'(' -f2 | cut -d')' -f1)";
      ...
      if [ -n "$vmIP" ]; then
        ...
        # fetch arp output for VM's IP
        tmp=$(/usr/sbin/arp -an | grep -i "$vmIP");
        
        # grep mac and make it lower case
        foundMac=$(echo $tmp | cut -d' ' -f4 | grep -v '<' | tr '[:upper:]' '[:lower:]');
...
and VM '$vhostName' with MAC='$mac' is still not available via SSH.";
  ...
  if $DEBUG; then
    ...
    #requires root rights :/ => solution may be setting chmod with metadata ?
    #logDebugMsg "Fetching syslog from '$vmIP'".
    #scp $SCP_OPTS $vmIP:$SYS_LOG_FILE $VM_JOB_DIR/$LOCALHOST/syslog_$vhostName.log;
  fi
